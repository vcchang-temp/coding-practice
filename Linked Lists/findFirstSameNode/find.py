# First same node in two lists
# Return first same node
# Assume guaranteed that same node exists
...
if __name__ == "__main__":
    ...
    expected = 4
    actual = find(head, head2)
    assert actual.val == expected
    print(f"Yay, found the first same node: {actual.val}")
    ...
    print(f"Yay, found the first same node: {actual}")