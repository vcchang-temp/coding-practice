...
def partition(arr: List, start: int, end: int):
    ...
    while leader < end:
        if arr[leader] <= arr[end]: # pick end == pivot; should be = to be stable sort
            arr[leader], arr[follower] = arr[follower], arr[leader] # building left subarray of elements smaller than pivot
            follower += 1
        leader += 1
...
if __name__ == "__main__":
    ...
    arr = ["a", "f", "c", "v", "k", "s", "d", "g"]
    quicksort(arr)
    assert arr == ["a", "c", "d", "f", "g", "k", "s", "v"]
    print(f"Wow ['a', 'c', 'd', 'f', 'g', 'k', 's', 'v'] was quickly sorted: {arr}")